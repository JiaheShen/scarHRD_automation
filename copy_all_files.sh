...
SRC_DIR="../"  # Modify this based on actual location
WORKFLOW_FILES_DIR="../workflow_files"  # Modify if needed
...
for sample_num in $(seq "$start_sample" "$end_sample"); do
    ...
    if [[ -d "$src_folder" ]]; then
        ...
        # Copy germline and normal BAM files and wait for completion
        copy_files "$sample_num" "$src_folder" "$dest_folder" "germline"
        copy_files "$sample_num" "$src_folder" "$dest_folder" "normal"

        # Copy workflow files only after BAM copies finish
